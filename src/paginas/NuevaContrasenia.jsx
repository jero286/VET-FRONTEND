...
const NuevaContrasenia = () => {
  ...
  const [confirmarNuevaContrasenia, setConfirmarNuevaContrasenia] = useState("");
  ...
  const handleClickFormNuevaContrasenia = async (ev) => {
    ...
    try {
      if (nuevaContrasenia === confirmarNuevaContrasenia) {
        const res = await clienteAxios.post(`/usuarios/changeNewPassUser?token=${token}`,
          { contrasenia: nuevaContrasenia }, configHeader
        )
      } else {
